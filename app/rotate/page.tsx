...
export default function RotatePage() {
    ...
    // Références pour le ZIP et la complétion
    ...
    const checkBatchCompletion = async (totalJobs: number) => {
        ...
        if (completedCount.current === totalJobs && totalJobs > 1) {
            ...
            try {
                ...
                a.href = url;
                a.download = "Fichiers_Pivotes.zip";
                a.click();
    ...
    const startSingleJob = async (job: JobState, index: number, totalJobs: number) => {
        ...
        try {
            const response = await fetch('/api/rotate', { method: 'POST', body: formData });
                ...
                if (data.status === 'done') {
                    ...
                    if (totalJobs === 1) {
                        ...
                        try {
                            ...
                            zipRef.current.file(`${baseName}-pivote.${ext}`, blob);
                        } catch (err) {
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
                <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        <span className="font-medium text-gray-700 whitespace-nowrap">Rotation :</span>
                        <select 
                            value={angle} 
                            onChange={(e) => setAngle(e.target.value)}
                            className="border-gray-300 text-black border p-2.5 rounded-lg bg-gray-50 outline-none w-full md:w-auto"
                    ...
                    >
                        {isProcessing ? 'Rotation en cours...' : 'Pivoter la sélection'}
                    </button>
                </div>
                    ...
                    <AnimatePresence mode="popLayout">
                        {jobs.map((job, i) => (
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                ) : ( <p className="text-xs text-gray-500 mt-1">Prêt pour la rotation</p> )}
                            </div>

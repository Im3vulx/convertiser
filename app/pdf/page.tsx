    ...
    const processFiles = (files: File[] | FileList) => {
        ...
        Array.from(files).forEach(file => {
            ...
            if (file.size > MAX_FILE_SIZE) {
                toast.error(`Trop volumineux (Max 50Mo) : ${file.name}`);
                return;
            }
    ...
    const checkBatchCompletion = async (totalJobs: number) => {
        ...
        if (completedCount.current === totalJobs && totalJobs > 1) {
            ...
            try {
                ...
                a.href = url;
                a.download = "Documents_PDF_Traites.zip";
                a.click();
                ...
                if (data.status === 'done') {
                    ...
                    if (totalJobs === 1) {
                        ...
                        try {
                            ...
                            const ext = targetFormat || 'pdf';
                            zipRef.current.file(`${baseName}-traite.${ext}`, blob);
                        } catch (err) {
        ...
        <div className="max-w-4xl mx-auto space-y-8">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
                <div className="flex flex-col gap-4">
                    ...
                    <button 
                        ...
                        className="w-full bg-black text-white py-3 rounded-xl font-bold transition-all hover:bg-gray-800 disabled:bg-gray-300 active:scale-[0.98]"
                    >
                        {isProcessing ? 'Traitement en cours...' : 'Traiter les documents'}
                    </button>
                </div>
            ...
            >
                ...
                <div className="pointer-events-none">
                    <h3 className="text-lg font-medium text-gray-900">Glissez-déposez vos PDF ici</h3>
                    <p className="text-sm text-gray-500 mt-1">ou cliquez pour parcourir (Max: 50 Mo par fichier)</p>
                </div>
            </div>
                ...
                <ul className="divide-y divide-gray-100">
                    <AnimatePresence mode="popLayout">
                        {jobs.map((job, i) => (
                            ...
                            >
                                <div className="w-14 h-14 bg-red-50 rounded-lg overflow-hidden flex items-center justify-center border border-red-100 shrink-0">
                                    <span className="text-2xl">📄</span>
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="font-medium text-sm text-gray-900 truncate">
                                        {job.file.name}
                                        <span className="ml-2 text-xs text-gray-400 font-normal">{formatFileSize(job.file.size)}</span>
                                    </p>
